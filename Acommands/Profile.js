...
module.exports.run = async (bot,message,args,DBprofile,DBstats,DBachievements,DBlevel,DBrole,DBidle) => {
    ...
    {
         ProfileID = player.id
         ProfileName = player.user.username
    }
    else{
    ...
    if(player)
    {
            if(!profile) return message.channel.send(`**:file_folder: | ${ProfileName} does not have a Profile yet**`)
    }
    else{
            if(!profile) return message.channel.send(`**Use \`${prefix}start\` to make a New Profile**`)
    }

    console.log(`Request for ${ProfileName} Profile`)
        ...
        if(!data.username)
        {
                return message.channel.send(`**:file_folder: | ${ProfileName} does not have a Profile yet**`)
        }
        ...
        TANE = emoji('661513315100393485') // Tankicoins
        ...
        IMG = 'https://en.tankiwiki.com/images/en/f/fc/Japan.png'
        let Embed = new Discord.RichEmbed()
        .setAuthor(ProfileName+' Profile',UserIcon)
